-- Safe first-run schema for Supabase.
-- This version avoids DROP statements so Supabase will not warn about destructive operations.
-- Use this on a fresh project. If you rerun it later, some CREATE statements may fail because objects already exist.
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'touch_site_sections_updated_at'
  ) then
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'touch_consultation_leads_updated_at'
  ) then
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'touch_admin_profiles_updated_at'
  ) then
